...
## Ad Works Light Weight (LT)

This example pattern shows data pipelines loading Adventure Works.

The data files can be seen here:

```
./data/landing/ad_works_lt
```
...
./ad_works_lt
    ...
    pipelines
        ...
        tables.yaml
    schema
        ... all the table schema files
    sql/control_ad_works_lt
        raw_audit.sql
```

This structure is described as follows from `./ad_works_lt`:

| path | description |
|-|-|
|ad_workad_works_lts.yaml                            | describes to yetl the project structure and project name |
...
|databricks/notebooks/bronze/create_audit_tables.py  | notebook to create the audit tables using the config in `.pipelines`|
...
|databricks/workflow/load_raw_ad_works_lt.json       | json definition of databricks workflow |
...
|pipelines/tables.xlsx                               | contains configuration that describes the tables and landing files to load in excel |
|schema                                              | contains the spark schema for schema on read or schema hinting when loading the landing files |
|sql/control_ad_works_lt                             | contains SQL table create scripts for tables we want to declare explicitly, in this case the audit tables we'll create |
...
- execute the notebook `./ad_works_lt/databricks/notebooks/setup.py`, this will clear down if you've run it previously or copy the landing data into the correct location.
- execute the notebook `./ad_works_lt/databricks/notebooks/bronze/create_audit_tables.py` with the default parameters, this will load the data using autoloader into the datalake house and audit tables. Note the following parameters:
- execute the notebook `./ad_works_lt/databricks/notebooks/bronze/load_raw.py` with the default parameters, this will load the data using autoloader into the datalake house and audit tables. Note the following parameters:
...
| load_type     | string: autoloader, batch | autoloader | The load pattern you want to use, there are 2 load patterns in this project. In `./pipelines/ad_works` there is `autoloader.yaml` and `batch.yaml` declared, use the file name of the pattern without the extension in parameter to switch between them.
...
- execute the notebook `./ad_works_lt/databricks/notebooks/setup.py`, to reset the landing data and clean up delta cataolog and file tables.
...
./ad_works
    ...
    schema
        ... all the table schema files
    sql/control_ad_works
        raw_audit.sql
```

This structure is described as follows from `./ad_works`:

| path | description |
|-|-|
|ad_works.yaml                                       | describes to yetl the project structure and project name |
|databricks                                          | contains all databricks assets |
|databricks/notebooks/setup.py                       | notebook to clear down and copy the demo data into landing |
|databricks/notebooks/checks.py                      | notebook to explore the data after loading |
|databricks/notebooks/bronze                         | contains notebooks for loading bronze or raw |
...
|databricks/notebooks/bronze/load_raw.py             | notebook to load all raw tables using the config in `.pipelines`, calls autoload_raw_schema.py|
|databricks/notebooks/etl                            | contains reusable python functions to load tables |
|databricks/workflow/load_raw_ad_works.json          | json definition of databricks workflow |
|pipelines/autoloader.yaml                           | contains a databricks workflow definition for executing the raw load using autoloader |
|pipelines/batch.yaml                                | contains a databricks workflow definition for executing the raw load using a batch pattern |
|pipelines/tables.yaml                               | contains configuration that describes the tables and landing files to load |
|pipelines/tables.xlsx                               | contains configuration that describes the tables and landing files to load in excel |
|schema                                              | contains the spark schema for schema on read or schema hinting when loading the landing files |
|sql/control_ad_works                                | contains SQL table create scripts for tables we want to declare explicitly, in this case the audit tables we'll create |
...
- clone this repo into databricks
- create and start a cluster using the latest cluster DBR LTS
- execute the notebook `./ad_works/databricks/notebooks/setup.py`, this will clear down if you've run it previously or copy the landing data into the correct location.
- execute the notebook `./ad_works/databricks/notebooks/bronze/create_audit_tables.py` with the default parameters, this will load the data using autoloader into the datalake house and audit tables. Note the following parameters:
- execute the notebook `./ad_works/databricks/notebooks/bronze/load_raw.py` with the default parameters, this will load the data using autoloader into the datalake house and audit tables. Note the following parameters:
...
- execute the notebook `databricks/notebooks/checks.py`, this has simple select statements to explore the data that was just loaded into the raw and audit control tables.
- execute the notebook `./ad_works/databricks/notebooks/setup.py`, to reset the landing data and clean up delta cataolog and file tables.
...
|databricks/notebooks/bronze/load_raw.py             | notebook to load all raw tables using the config in `.pipelines`, calls autoload_raw_schema.py|
|databricks/notebooks/etl                            | contains reusable python functions to load tables |
|databricks/workflow/load_raw_header_footer.json     | json definition of databricks workflow |
|pipelines/autoloader.yaml                           | contains a databricks workflow definition for executing the raw load using autoloader |
|pipelines/batch.yaml                                | contains a databricks workflow definition for executing the raw load using a batch pattern |
|pipelines/tables.yaml                               | contains configuration that describes the tables and landing files to load |
|schema                                              | contains the spark schema for schema on read or schema hinting when loading the landing files |
